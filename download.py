...
data = requests.get("https://webapp.scuolabook.it/books/" +
                   str(bookid) + ".json", headers=header).text
...
for match in matches:
   sys.stdout.write("\rDownloading page " +
                    str(matches.index(match) + 1) + "/" + str(npages) + "...")
   sys.stdout.flush()
